...
class GoodsServiceImpl(ServiceImpl[GoodsMapper, GoodsDO], GoodsService):
    ...
    async def redis_deploy(self, redisDeployCmd: RedisDeployCmd):
        ...
        try:
            temp_dir, inventory_file_path, site_file_path, global_var_file_path, role_names = generate_playbook_paths(
                zip_file_object)
            role_name = role_names[0]
            async with db_session() as session:
                inventoryRecords = await inventoryMapper.select_records_by_ids(ids=redisDeployCmd.host_ids,
                                                                               db_session=session)
            ...
            global_content = global_template.render(redis_version=redisDeployCmd.version,
                                                    redis_password=redisDeployCmd.password)
            with open(global_var_file_path, 'w', encoding='utf-8') as file:
                file.write(global_content)
            command = ["ansible-playbook", "-i", inventory_file_path, "-e", f"@{global_var_file_path}", site_file_path]
